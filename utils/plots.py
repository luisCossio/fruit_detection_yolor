...
def plot_images2(images, targets, paths=None, fname='images.jpg', names=None,
                 max_size=1024):
    tl = 3  # line thickness
    tf = max(tl - 1, 1)  # font thickness
    ...
    # Save only the first example, without size reduction
    img = images[0]
    img = img.transpose(1, 2, 0)
    img = img.copy().astype(dtype=np.uint8)
    ...
    if paths is not None:
        label = os.path.basename(paths[0])[:40]  # trim to 40 char
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
