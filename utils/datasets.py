    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    # path_images = os.path.join(p,'images')
                    # f += [os.path.join(path_images,file) for file in sorted(os.listdir(path_images))]
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels1.size > 0:
                ...
                labels = labels1.copy()
                labels[:, 1] = ratio[0] * w * (labels1[:, 1] - labels1[:, 3] / 2) + pad[0]  # pad width
            ...
            if random.random() < hyp['mixup']:
                ...
                labels_2 = self.labels[index2]

                labels2 = None
                if labels_2.size > 0:
                    # Normalized xywh to pixel xyxy format
                    labels2 = labels_2.copy()
                    labels2[:, 1] = ratio2[0] * w * (labels_2[:, 1] - labels_2[:, 3] / 2) + pad[0]  # pad width
                    ...
                    if labels is None:
                        ...
                        labels = np.concatenate((labels, labels2), 0)
                r = np.random.beta(8.0, 8.0)  # mixup ratio, alpha=beta=8.0 , value between 0.0 and 1.0
                # r = 0.5
                img = (img * r + img2 * (1 - r)).astype(np.uint8)
...
path = '/home/luis/datasets/minneapple/val/images'
img_size=1280
batch_size=3
augment = True
hyp = {'lr0': 0.01, 'momentum': 0.937, 'weight_decay': 0.0004921875, 'giou': 0.05,
         ...
         'degrees': 0.0, 'translate': 0.00, 'scale': 0.00, 'shear': 0.0, 'perspective': 0.0,
